...
先写redolog，再更新内存，根据时机再从redolog同步到磁盘，因为直接写磁盘还需要找到数据具体的位置。
...
最好选自增主键作为索引，也有适合用业务字段做主键的情况。



#### 覆盖索引

指我们索引树本身已经包含了我们想要的信息，比如我们普通索引存的是主键值，那如果我们的查的就是主键，就直接拿到了。

或者我们建了个联合索引，比如身份证+姓名的联合索引。如果有通过身份证查姓名的需求，这个索引就是合适的。

---

再进一步，如果既有联合索引，又需要单独的索引，比如有(a,b)又要单独查b, 根据最左前缀，单查b是不行的，这个时候就可以根据具体空间的情况，比如a是int,b是varchar，那就建立(a,b) 和a索引比较合适
...
可重复读的原理是，事务开始时会创建一个静态视图，事务中的操作以当前视图为准。
...
即使是对于小表的更改也要注意，比如读的时候，你想增加表字段，这个时候会被阻塞，因为被写锁阻塞，那其它读的操作过来时也会被阻塞。

ALTER TABLE tbl_name WAIT N add column ...（加上等待时间，如果指定时间没拿到锁就放弃）

#### 行锁

事务A执行两条更新语句，那就会持有两条记录的行锁，这个锁是需要的时候加上的，但并不是用完就释放的，是在事务commit的时候才释放的。

所以如果你的事务中如果需要持有多把锁，那尽量把对并发影响最大的放在后面执行。比如更新日志，更新用户数据，更新总数据。

#### 死锁解决

- 一种策略是，直接进入等待，直到超时。这个超时时间可以通过参数innodb_lock_wait_timeout来（默认值是50s）设置。
- 另一种策略是，发起死锁检测，发现死锁后，主动回滚死锁链条中的某一个事务，让其他事务得以继续执行。将参数innodb_deadlock_detect设置为on，表示开启这个逻辑。
  - 如果团队有数据库的专家可以改mysql的源码，就是让对于行更新的操作在进入引擎
  - 也可以将一行改成逻辑上的多行，比如库存分成多行，最后加总是总数，但是对于负数的出现要注意。

方案1的时间不好控制，通常采用第二种。但第二种会产生热点数据导致的死锁检测消耗大量CPU资源，但整个数据库每秒才执行不到100个事务。



#### 快照

InnoDB里面每个事务有一个唯一的事务ID，叫作transaction id，是严格递增的。快照只是逻辑上的，不是说把当前所有的数据都存了一个副本。因为事务是有顺序的，所以实际上值是推算出来的，r根据事务顺序，找到它是undolog计算出来。每个版本对应一个transaction id。

实现上的话，InnoDB为每个事务构造了一个数组，保存事务启动瞬间当前活跃(未提交)的所有事务id，最小值为低水准，最高+1为高水准。低于低水准的就是可见的，高于高水准是不可见的。中间的

都是代码逻辑转译过来的，人肉分析确实很麻烦。
