...
普通情况下先通过张三找到所有匹配数据的主键，再从中找到匹配age信息的返回

索引下推就是说，我们原先匹配过程中，组合索引的第二个其实是被浪费了，在匹配name的同时匹配age=10的情况，实际只需要将name、age同时匹配的数据中储存的主键返回即可，这样可以减少回表的次数。
...
* 用了bigkey,这样redis分配内存和删除key都会比较耗时 
* 大量key同时过期
  * 第一种就是在业务中加随机过期时间
    第二种应用于4.0后的redis，开启lazy-free
* 实例内存到达上限
  * allkeys-lru
    视情况可以改为随机淘汰，比lru快很多

* fork耗时严重
  * 需要控制redis实例的大小，不然fork操作的耗时会很大

* 开启内存大页
  * 同样是fork的时候，由于开启了写时复制机制。主进程fork出了子进程之后，主进程依旧可以接收写请求，就是写时复制。也就是说有数据需要修改的时候，是写入一块新的内存。加入我现在只需要写一个10B的数据，但是由于开启了内存大页，redis以2M为单位向操作系统申请。
...
它会把所有发送到该Exchange的消息路由到所有与它绑定的Queue中，不需要做任何判断操作，所以 fanout 类型是所有的交换机类型里面速度最快的。fanout 类型常用来广播消息
...
容器更加轻量，因为容器不像虚拟机，是没有自己的内核的，也没有硬件虚拟。容器内的进程直接运行于宿主的内核
...
按照 Docker 最佳实践的要求，**容器不应该向其存储层内写入任何数据** ，容器存储层要保持无状态化。这样容器使用数据卷后可以随意删除，重新run，数据却不会丢失。
...
如果请求中有没有订单id,就需要新建一个订单，如果有订单号说明查过，那就只需要预约的类型,同时也需要更新计价规则。

> 这里需要考虑到重复访问的问题，不能说每次访问我都要去走一遍创建订单的流程

* 不是日租和半日租的话，调用PrepareTask去获取路途长度和行驶时间
